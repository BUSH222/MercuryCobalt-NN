...
def validate(s: dict) -> None:
    ...
    if not (
        0 <= e["min_elevation_deg"] < 90 and 0 < e["isl_range_km"] <= 10000 and (0 <= e["target_availability"] <= 1)
    ):
    ...
    for field, key, valid in [
        ...
        ("gateway_outages", "gateway_id", {g["id"] for g in ground if g["role"] == "gateway"}),
    ]:
        for f in s[field]:
            if (
                f[key] not in valid
...
def snapshot(s: dict, t_s: float) -> dict:
    ...
    e, d = (s["environment"], s["design"])
    ids, _, xyz = positions(s, t_s)
    failed = {f["satellite_id"] for f in s["failures"] if f["start_s"] <= t_s < f["end_s"]}
    active = np.array(
        [sat["launch_batch"] <= d["launch_stage"] and sat["id"] not in failed for sat in d["satellites"]]
    )
    ...
    edges = [[ids[a], ids[b], float(dd)] for a, b, dd in zip(i[ok], j[ok], dist[ok], strict=False)]
    elevations = {}
